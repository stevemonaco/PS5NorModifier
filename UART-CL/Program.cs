...
if (!Globals.DatabaseService.IsLocalDatabaseAvailable())
{
    ShowHeader();
    Console.WriteLine("Downloading latest database file. Please wait...");
    var success = await Globals.DatabaseService.DownloadRemoteDatabase("http://uartcodes.com/xml.php", "errorDB.xml");
...
static async Task RunSubMenu(string appTitle)
{
    string pathToDump = "";
    bool subMenuRunning = true;
    BiosDump? biosDump = null;
        ...
        switch (Console.ReadLine())
        {
            case "1": // Load a BIOS dump
                ...
                do
                {
                    ...
                    else if (!Path.GetExtension(userInput).Equals(".bin", StringComparison.OrdinalIgnoreCase))
                    {
                    ...
                    else
                    {
                        ...
                        Console.Write("Loading bios...");
                        biosDump = await Globals.BiosService.LoadBiosDump(pathToDump);
            ...
            case "2": // View BIOS information
                if(biosDump is not null)
                {
                    // Start showing the data we've found to the user
                    var biosLength = biosDump.Raw.Length;
                    var biosLengthMB = biosLength / 1024 / 1024;

                    Console.WriteLine($"File size: {biosLength} bytes ({biosLengthMB} MB)"); // The file size of the .bin file
                    Console.WriteLine($"PS5 Version: {biosDump.Edition}"); // Show the version info (disc/digital/slim)
                    Console.WriteLine($"Console Model: {biosDump.ModelInfo}"); // Show the console variant (CFI-XXXX)
                    Console.WriteLine($"Console Serial Number: {biosDump.ConsoleSerialNumber}"); // Show the serial number. This serial is the one the disc drive would use
                    Console.WriteLine($"Motherboard Serial Number: {biosDump.MotherboardSerialNumber}"); // Show the serial number to the motherboard. This is different to the console serial
                    Console.WriteLine($"WiFi Mac Address: {biosDump.WiFiMac}"); // WiFi mac address
                    Console.WriteLine($"LAN Mac Address: {biosDump.LANMac}"); // LAN mac address
                    // Just a blank line
                    Console.WriteLine("");
                ...
                {
                    PrintBiosNotLoadedWarning();
                }

                break;

            case "3": // Convert to "Digital" edition
                if (biosDump is not null)
                {
                    ConvertToEdition(biosDump, Ps5Edition.Digital, pathToDump);
                }
                else
                {
                    PrintBiosNotLoadedWarning();
            ...
            case "4": // Convert to "Disc" edition
                if (biosDump is not null)
                {
                    ConvertToEdition(biosDump, Ps5Edition.Digital, pathToDump);
                }
            ...
            case "5": // Convert to "Slim" edition
            ...
            case "7": // Change motherboard serial number
                if (!string.IsNullOrEmpty(pathToDump)) // If the pathToDump string isn't empty or null, we can try and work with it
                {
...
static async Task<bool> MainMenu(string appTitle)
{
    ...
    switch (Console.ReadLine())
    {
                    ...
                    switch (split[0])
                    {
                        ...
                        case "OK":
                            ...
                            else
                            {
                                ...
                                Console.ForegroundColor = ConsoleColor.Green;
                                string errorResult = Globals.DatabaseService.ParseErrors(errorCode);
        ...
        case "7":
            ...
            bool success = await Globals.DatabaseService.DownloadRemoteDatabase("http://uartcodes.com/xml.php", "errorDB.xml");
...
static void ConvertToEdition(BiosDump dump, Ps5Edition edition, string saveLocation)
{
    bool confirmed = false;
    while (!confirmed)
    {
        Console.ForegroundColor = ConsoleColor.Blue;
        Console.WriteLine($"Are you sure you want to set the console as \"{edition}\" edition?");
        Console.ResetColor();
        ...
        if (changeConfirmation == "yes")
        {
            ...
            else
            {
                ...
                try
                {
                    dump = dump with { Edition = edition };
                    ...
                    foreach (int index in Helpers.PatternAt(dump.Raw, find))
                    {
                        for (int i = index, replaceIndex = 0; i < dump.Raw.Length && replaceIndex < replace.Length; i++, replaceIndex++)
                        {
                            dump.Raw[i] = replace[replaceIndex];
                        }
                        File.WriteAllBytes(saveLocation, dump.Raw);
                    }
                    ...
                    foreach (int index in Helpers.PatternAt(dump.Raw, find2))
                    {
                        for (int i = index, replaceIndex = 0; i < dump.Raw.Length && replaceIndex < replace.Length; i++, replaceIndex++)
                        {
                            dump.Raw[i] = replace2[replaceIndex];
                        }
                        File.WriteAllBytes(saveLocation, dump.Raw);
                    }

                    Console.WriteLine("Your BIOS file has been updated successfully. The new .bin file will now report to");
                    Console.WriteLine($"the PlayStation 5 as a '{edition} edition' console.");
                    Console.WriteLine("Press Enter to continue...");
                    Console.ReadLine();
                    confirmed = true;
                    break;
...
static void PrintBiosNotLoadedWarning()
{
    Console.ForegroundColor = ConsoleColor.Red;
